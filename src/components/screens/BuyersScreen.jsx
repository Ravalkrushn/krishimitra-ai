...
function ScoreDonut({ score, color }) {
  ...
  return (
    <svg width="54" height="54" viewBox="0 0 54 54">
      ...
      />
      <text x="27" y="31" textAnchor="middle" fontSize="11" fontWeight="800" fill="#1f2328">{score}%</text>
    </svg>
  );
}
  ...
  return (
    <div className="screen">
      ...
      <div className="bs-header-row">
        <div>
          <h2 className="bs-title">Find Buyers</h2>
          <p className="bs-sub">Connect with verified buyers and get the best price for your produce.</p>
        </div>
        <span className="demo-badge demo-badge-gold">🔆 DEMO DATA</span>
      </div>

      {/* ── Requirement Summary Bar ── */}
      <div className="card card-pad bs-req-bar mb-16">
        <div className="bs-req-item">
          <span className="bs-req-icon">🌿</span>
          <div>
            <div className="bs-req-label">Crop</div>
            <div className="bs-req-val">{crop.charAt(0).toUpperCase() + crop.slice(1)}</div>
          </div>
        </div>
        <div className="bs-req-item">
          <span className="bs-req-icon">💰</span>
        ...
        <button className="bs-edit-btn">✏️ Edit Requirement</button>
      </div>
      ...
      <div className="bs-search-row mb-16">
        ...
        <button className="bs-filter-btn">▼ Filters</button>
      </div>

      {/* ── Buyer Cards ── */}
      <div className="bs-cards-stack">
        {buyers.map((b) => (
          <div key={b.rank} className={`card bs-buyer-card bs-match-${b.matchColor}`}>
            {/* Rank badge */}
            <div className={`bs-rank-badge bs-rank-${b.matchColor}`}>{b.rank}</div>

            {/* Match chip + name */}
            <div className="bs-card-header">
              <span className={`bs-match-chip bs-match-chip-${b.matchColor}`}>
                {b.matchColor === "green" ? "⭐" : "🔆"} {b.matchLevel}
              </span>
            </div>
            <div className="bs-card-name-row">
              <div className="bs-card-icon">🏢</div>
              <div>
                <div className="bs-card-name">{b.name} <span className="bs-demo-tag">Demo Buyer</span></div>
                <div className="bs-card-loc">📍 {b.location} &nbsp;•&nbsp; {b.distDisplay} from you</div>
              </div>
            </div>
            ...
            <div className="bs-details-row">
              ...
              <div className="bs-detail-item">
                ...
                <div className="bs-dv bs-price">₹{b.price.toLocaleString("en-IN")}<span className="bs-per">/Quintal</span></div>
              </div>
              <div className="bs-detail-item">
            ...
            <div className="bs-reasons-row">
              <div className="bs-reasons-left">
                ...
                <div className="bs-reason-chips">
                  {b.reasons.map((r, i) => (
                    <span key={i} className="bs-reason-chip">✓ {r}</span>
                  ))}
                </div>
              </div>
              <button className="bs-contact-btn">View Details &amp; Contact</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="bs-ai-rec-body">
          <div className="bs-ai-rec-left">
            ...
            <p className="bs-ai-rec-desc">
              This buyer offers the best price ₹{BEST_BUYER.price.toLocaleString("en-IN")}/quintal for your {qty} quintals of cotton.
              They are located {BEST_BUYER.dist} km from you and match your quality requirement (SG-2 or Better) perfectly.
            </p>
          </div>
          <div className="bs-ai-rec-right">
            <div className="bs-ai-stat">
              <div className="bs-ai-stat-l">Offered Price</div>
              <div className="bs-ai-stat-v">₹{BEST_BUYER.price.toLocaleString("en-IN")} /Quintal</div>
            </div>
            <div className="bs-ai-stat">
              <div className="bs-ai-stat-l">Expected Value</div>
              <div className="bs-ai-stat-v">₹{(BEST_BUYER.price * parseFloat(qty)).toLocaleString("en-IN")}</div>
            </div>
            <div className="bs-ai-stat">
            ...
            <button className="bs-contact-buyer-btn">📞 Contact This Buyer</button>
          </div>
        </div>
      </div>
      ...
      <div className="bs-timestamp">
        <span>🕐 Data as on: 17 Jun 2025, 09:30 AM</span>
        <span>Buyers are shown based on your requirement and AI matching</span>
      </div>

      {/* ── Disclaimer ── */}
      <div className="disclaimer bs-disclaimer">
        <span className="bs-disc-icon">🛡️</span>
        <span>
          <strong style={{color:"#b45309"}}>Disclaimer:</strong> All buyers and prices shown here are{" "}
          <strong>DEMO DATA</strong> for educational purposes only. Actual market prices and buyer details may vary.
        </span>
      </div>
    </div>
  );
